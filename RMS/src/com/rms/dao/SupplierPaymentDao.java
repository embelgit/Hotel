...
import com.rms.bean.SupplierPaymentDetail;
import com.rms.hibernate.SupplierPaymentBean;
...
public class SupplierPaymentDao {
	...
	public List<SupplierPaymentDetail> getSupplierUnpaidPaymentDetailsAsBill(
			String fkSupplierId) {
		...
		try
		{
				hbu = HibernateUtility.getInstance();
		 session = hbu.getHibernateSession();
		 Query query = session.createSQLQuery("select supplier_name, bill_no, balance FROM supplier_details RIGHT JOIN supplier_payment ON supplier_details.supplier_id = supplier_payment.fk_supplier_id WHERE supplier_payment.fk_supplier_id=:fkSupplierId");
			query.setParameter("fkSupplierId", fkSupplierId);
			List<Object[]> list = query.list();
			supplierList = new ArrayList<SupplierPaymentDetail>(0);
