...
import com.rms.bean.CreditCustPaymentDetail;
import com.rms.hibernate.CustomerPaymentBean;
...
// run when credit customer bill have done first time
	...
	public List<CreditCustPaymentDetail> getCreditCustomerBalanceReportNameWise(
			String fkCustomerId) {
		...
		try
		{
				hbu = HibernateUtility.getInstance();
		 session = hbu.getHibernateSession();
		 Query query = session.createSQLQuery("select bill_no, balance FROM customer_details RIGHT JOIN credit_customer_payment ON customer_details.pk_customer_id = credit_customer_payment.fk_customer_id WHERE credit_customer_payment.fk_customer_id=:customer");
			query.setParameter("customer", fkCustomerId);
			List<Object[]> list = query.list();
			customerList = new ArrayList<CreditCustPaymentDetail>(0);
