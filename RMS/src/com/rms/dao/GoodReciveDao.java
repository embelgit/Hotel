...
import com.rms.bean.PurchaseReportBean;
...
public class GoodReciveDao {
	...
	public void updateQuantity(Long item_id,String quantity) {
		...
		try
		{
		    ...
		    Double updatequnty = (Double) (quant - Double.parseDouble(quantity));
	...
	public List<GoodReceive> getCategoryWiseStock(Long barcodeNo) {
		...
		try
		{
			hbu = HibernateUtility.getInstance();
		 session = hbu.getHibernateSession();
		 Query query2 = session.createQuery("select itemName, catName, quantity, buyPrice, billNo, barcodeNo, vat, igst from GoodReceive where barcodeNo=:barcodeNo");
...
//get purchase bill no
	...
	public List<PurchaseReportBean> purchaseReportBetweenTwoDates(Date adate,
		...
		{   Long k = 0l;
			hbu = HibernateUtility.getInstance();
		 session = hbu.getHibernateSession();
		 Query query2 = session.createSQLQuery("select g.Date, s.supplier_name, g.BillNo, s.pan_no, g.ItemName, g.BuyPrice, g.OrignalQuantity, g.vat, g.igst, g.HsnSacNo from goodreceive g left join supplier_details s on g.FksuppId = s.supplier_id where date BETWEEN :adate AND :bdate");
		 query2.setParameter("adate", adate);
	...
	public List<PurchaseReportBean> purchaseReportBetweenTwoDatesNonGst(Date adate,
			Date bdate) {
		...
		// Query query2 = session.createQuery("select billNo, carNo, barcodeNo, itemName, categoryName, salePrice, ownerName, contactNo, totalAmt, discount from CustomerBill where current_date=:adate");
		 ...
		 query2.setParameter("billNo", billNo);
			...
			for (Object[] object : list) {
				...
				reports.setExpence(Double.parseDouble(object[13].toString()));
				/*reports.setTxPerexpence(Double.parseDouble(object[14].toString()));*/
				reports.setGrossTotal((double) Math.round(Double.parseDouble(object[14].toString())));
	...
	//SupplierName Wise Purchase Report 
		public List<PurchaseReport> suppliernamewiseAllPurchase(String supplier) {
			...
			try
			{
			 ...
			 Long k = 0l;
			// Query query2 = session.createQuery("select billNo, carNo, barcodeNo, itemName, categoryName, salePrice, ownerName, contactNo, totalAmt, discount from CustomerBill where current_date=:adate");
			 Query query2 = session.createSQLQuery("select s.supplier_name, g.BillNo, g.CategoryName, g.ItemName, g.ContactPerson, g.Quantity, g.BuyPrice, g.Vat, g.Total, g.Expences, g.Date, g.HsnSacNo, g.Igst, g.TaxAmount, g.GrossTotal from goodreceive g inner join supplier_details s on g.FksuppId = s.supplier_id where FksuppId=:supplier");
			 query2.setParameter("supplier", supplier);
				...
				for (Object[] object : list) {
					...
					reports.setTotal((double) Math.round(Double.parseDouble(object[8].toString())));
					reports.setExpence(Double.parseDouble(object[9].toString()));
					reports.setGrossTotal((double) Math.round(Double.parseDouble(object[14].toString())));
