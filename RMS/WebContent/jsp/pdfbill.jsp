...
System.out.println("in pdf jsp tableno wise user- "+username+" , id - "+userid+" , hotelnme - "+hotelname+" , hotelid - "+hotelid);
%>
...
<%
	response.setContentType("application/pdf");
	 ...
	 System.out.println("bill no got initially - - "+billno);
	 final String FONT ;
	...
	//	double gstAmt = 0;
		...
		try {
			...
			Document document = new Document(PageSize.A6, 10f,10f, 0f, 10f);
 ...
 */			
			...
			float[] columnWidths = { 0.9f, 0.45f, 0.8f,0.55f,0.55f, 0.8f };
			...
			table_cell = new PdfPCell(new Phrase("ItemName",font12));
...
/* 			table_cell = new PdfPCell(new Phrase("GST",font12));
	...
	/* 			String tableNo = String.valueOf(rs.getString("TableNo"));
				...
				String CGST = String.valueOf(rs.getString("CGST"));
				table_cell = new PdfPCell(new Phrase(new Phrase("\n " + CGST)));
				table_cell.setHorizontalAlignment(Element.ALIGN_LEFT);
				table_cell.setBorder(Rectangle.NO_BORDER | Rectangle.NO_BORDER);
				table.addCell(table_cell);
				
				String SGST = String.valueOf(rs.getString("SGST"));
				table_cell = new PdfPCell(new Phrase(new Phrase("\n " + SGST)));
				table_cell.setHorizontalAlignment(Element.ALIGN_LEFT);
				table_cell.setBorder(Rectangle.NO_BORDER | Rectangle.NO_BORDER);
				table.addCell(table_cell);
		...
		/* 		double price = rs.getDouble("totalperitem"); */
				...
				double cgst =  rs.getLong("CGST");
				double sgst = rs.getLong("SGST");
				double totc = (cgst/100)*taxperitem;
				double tots  = (sgst/100)*taxperitem;
				double taxamt = taxperitem + totc+ tots;
				
				long taxamtl = Math.round(Double.valueOf(taxamt));
...
//				table_cell = new PdfPCell(new Phrase("\n " + GrandAmtss));
			...
			/* 	table_cell = new PdfPCell(new Phrase(new Phrase("\n " + GST2)));
				...
				table.addCell(table_cell); */
			    GST2 = Double.valueOf(rs.getDouble("GST"));
				String tot = String.valueOf(rs.getString("GridTotal"));
				gsttotal = (finalgross*(GST2/100));
				gstAmt = Double.valueOf(tot);
			}
...
//			Double GST = Double.valueOf(rs.getDouble("GST"));
//			table_cell = new PdfPCell(new Phrase("GST : "+GST));
/*			InfoTable_cell.setHorizontalAlignment(Element.ALIGN_LEFT);
*/			
	//		table_cell = new PdfPCell(new Phrase(rs.getString(GST), font14));		
/*			table_cell = new PdfPCell(new Phrase("GST : "+GST));			
			table_cell.setColspan(4);
			table_cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
			table_cell.setBorder(Rectangle.NO_BORDER);
			table.addCell(table_cell);
	*/		
	//	 GST1 = Double.valueOf(rs.getDouble("GST"));
	
//	String gst = String.valueOf(rs.getString("GST"));	
					
		//		table_cell = new PdfPCell(new Phrase(new Phrase("Total : " + finalgross)));
		//		table_cell.setColspan(4);
		//		table_cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
		//		table_cell.setBorder(Rectangle.NO_BORDER);
		//		table.addCell(table_cell); 
			
		/*		table_cell = new PdfPCell(new Phrase(new Phrase("GST % : " + GST2)));
	//			table_cell.setColspan(4);
				table_cell.setHorizontalAlignment(Element.ALIGN_LEFT);
				table_cell.setBorder(Rectangle.NO_BORDER);
				table.addCell(table_cell); 
				
				long longGrandTotalAmtt = Math.round(Double.valueOf(gsttotal));
				String strGrandTotalAmtt = String.valueOf(longGrandTotalAmtt);
				double intGrandTotalAmtt = Integer.parseInt(strGrandTotalAmtt);
				table_cell = new PdfPCell(new Phrase(new Phrase("GST Amount : " + intGrandTotalAmtt)));
				table_cell.setColspan(4);
				table_cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
				table_cell.setBorder(Rectangle.NO_BORDER);
				table.addCell(table_cell); */
				...
				long longGrandAmtt = Math.round(Double.valueOf(gstAmt));
				String strGrandAmtt = String.valueOf(longGrandAmtt);
				double GrandAmt = Integer.parseInt(strGrandAmtt);
			table_cell = new PdfPCell(new Phrase(df.format(finalgross), font14));		
			table_cell = new PdfPCell(new Phrase("Grand Total : "+GrandAmt, font14));			
			table_cell.setColspan(4);
			table_cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
			table_cell.setBorder(Rectangle.NO_BORDER);
			table.addCell(table_cell);
...
//			table_cell = new PdfPCell(new Phrase(df.format(finalgross), font14));
			...
			long longGrandTotalAmt = Math.round(Double.valueOf(gstAmt));
