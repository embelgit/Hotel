...
System.out.println("in pdf jsp user- "+username+" , id - "+userid+" , hotelnme - "+hotelname+" , hotelid - "+hotelid);
%>
...
//			ResultSet rs = stmt.executeQuery("select c.TableNo, c.Itemname, c.Quantity, c.unitPrice, c.CurrentDate, c.GST from customerorder c where c.BillNo ='" + billno+"' and c.hotelid ='"+hotelid+"' and userid = '"+userid+"'");
...
//			table_cell.setColspan(4);
			...
			long TotalAmt = Math.round(Double.valueOf(gsttotal));
			String TotalAt = String.valueOf(TotalAmt);
			double TotalAmtt = Double.parseDouble(TotalAt);
			table_cell = new PdfPCell(new Phrase(new Phrase("GST Amount : " + TotalAmtt)));
			table_cell.setColspan(4);
			table_cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
			table_cell.setBorder(Rectangle.NO_BORDER);
			table.addCell(table_cell); 
			*/
			...
			long TotalA = Math.round(Double.valueOf(gstAmt));
			String TotalAttt = String.valueOf(TotalA);
			double GTotalAmtt = Double.parseDouble(TotalAttt);
			table_cell = new PdfPCell(new Phrase(df.format(finalgross), font14));		
			table_cell = new PdfPCell(new Phrase("Grand Total : "+GTotalAmtt, font14));			
			table_cell.setColspan(4);
			table_cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
			table_cell.setBorder(Rectangle.NO_BORDER);
			table.addCell(table_cell);
