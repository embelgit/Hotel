...
<%
	response.setContentType("application/pdf");
	 ...
	 System.out.println("bill no in pdf - - "+billno);
...
//			ResultSet rs = stmt.executeQuery("select c.TableNo, c.Itemname, c.Quantity, c.unitPrice, c.CurrentDate  from customerorder c where c.BillNo =" + billno);
			...
			ResultSet rs = stmt.executeQuery("select c.TableNo, c.Itemname, c.Quantity, c.unitPrice, c.CurrentDate, c.GST from customerorder c where c.BillNo =" + billno);
...
//			table_cell.setColspan(4);
			...
			table_cell = new PdfPCell(new Phrase(new Phrase("GST Amount : " + gsttotal)));
			table_cell.setColspan(4);
			table_cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
			table_cell.setBorder(Rectangle.NO_BORDER);
			table.addCell(table_cell); 
			
			table_cell = new PdfPCell(new Phrase(" \n Grand Total : "+gstAmt, font14));
			table_cell.setColspan(4);
			table_cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
			table_cell.setBorder(Rectangle.NO_BORDER);
			table.addCell(table_cell);
